...
queries_df = pd.read_csv(os.path.join(os.getcwd(), "data", "dataset_queries.csv"), sep="\t")
print(queries_df)
    ...
    if res["hits"]["hits"]:
        ...
        for rd in res["hits"]["hits"]:
            if d["id"] != rd["_source"]["id"]:
                temp_pairs += [{"id1": x[0], "id2": y[0], "lem_query1": x[1], "lem_query2": y[1]} 
                                  for x, y in [tuple(sorted([(d["id"], d["lem_query"]), (rd["_source"]["id"], rd["_source"]["lem_query"])]))]]
        not_similar_queries += temp_pairs[:5]
...
print(not_similar_queries_df.shape)
not_similar_queries_df.to_csv(os.path.join(os.getcwd(), "data", "dataset_dissimilar_queries.csv"), sep="\t", index=False)