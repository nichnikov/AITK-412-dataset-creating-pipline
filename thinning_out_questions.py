...
for sys_id in sys_ids:
    fa_ids = clusters_df[clusters_df["SysID"]==sys_id]["ID"].unique()
    for num, fa_id in enumerate(fa_ids):
        logger.info(str("sys:" + str(sys_id) + " " + str(num) + "/" + str(len(fa_ids))))
        quries = list(clusters_df["Cluster"][(clusters_df["SysID"]==sys_id) & (clusters_df["ID"] == fa_id)])
        if len(quries) <= 300:
    ...
    print(queries_df)
    queries_df.to_feather(os.path.join("data", "240212", "thinning_clusters_2.feather"))